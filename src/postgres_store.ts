...
const POSTGRES_MIGRATIONS: string[] = [
  `ALTER TABLE mg_entity_fact ADD COLUMN IF NOT EXISTS emotional_weight DOUBLE PRECISION`,
  `ALTER TABLE mg_entity_fact ADD COLUMN IF NOT EXISTS emotional_valence TEXT`,
  `ALTER TABLE mg_entity_fact ADD COLUMN IF NOT EXISTS verbatim TEXT`,
  `ALTER TABLE mg_entity_fact ADD COLUMN IF NOT EXISTS started_at TEXT`,
...
export class PostgresStore implements Store {
  ...
  private async createSchema(): Promise<void> {
    ...
    // Run migrations for existing databases (ADD COLUMN IF NOT EXISTS is safe to re-run).
  ...
  async listFactsMetadata(entityUuid: string, filter: FactFilter, limit: number): Promise<Fact[]> {
    ...
    if (filter.threadStatuses && filter.threadStatuses.length > 0) {
      const placeholders = filter.threadStatuses.map(() => `$${paramIdx++}`).join(',');
      query += ` AND thread_status IN (${placeholders})`;
      args.push(...filter.threadStatuses);
    }
    if (filter.pinned !== undefined) {
      query += ` AND pinned = $${paramIdx++}`;
      args.push(filter.pinned ? 1 : 0);
    }
    if (filter.emotionalValences && filter.emotionalValences.length > 0) {
      const placeholders = filter.emotionalValences.map(() => `$${paramIdx++}`).join(',');
      query += ` AND emotional_valence IN (${placeholders})`;
  ...
  async pinFact(factUuid: string): Promise<void> {
    ...
    const now = nowISO();
    await this.pool.query(
      `UPDATE mg_entity_fact SET pinned = 1, significance = 10, expires_at = NULL, updated_at = $1 WHERE uuid = $2`,
      [now, factUuid]
    );
  }
  ...
  async unpinFact(factUuid: string): Promise<void> {
    ...
    const now = nowISO();
    await this.pool.query(
      `UPDATE mg_entity_fact SET pinned = 0, updated_at = $1 WHERE uuid = $2`,
      [now, factUuid]
    );
  }
  ...
  async confirmFact(factUuid: string): Promise<void> {
    ...
    const now = nowISO();
    await this.pool.query(
      `UPDATE mg_entity_fact SET confidence = 1.0, reinforced_at = $1, reinforced_count = reinforced_count + 1, updated_at = $2 WHERE uuid = $3`,
      [now, now, factUuid]
    );
  }
  ...
  async updateThreadStatus(factUuid: string, status: string): Promise<void> {
    ...
    const now = nowISO();
    await this.pool.query(
      `UPDATE mg_entity_fact SET thread_status = $1, updated_at = $2 WHERE uuid = $3`,
      [status, now, factUuid]
    );
  }
  ...
  async listOpenThreads(entityUuid: string, limit: number): Promise<Fact[]> {
    ...
    const res = await this.pool.query(
      `SELECT ${FACT_COLUMNS} FROM mg_entity_fact WHERE entity_id = $1 AND thread_status = 'open' ORDER BY significance DESC, created_at DESC LIMIT $2`,
      [entityUuid, effectiveLimit]
    );
    return res.rows.map(rowToFact);
  }
  ...
  async updateEmotionalWeight(factUuid: string, weight: number, valence: string): Promise<void> {
    ...
    const now = nowISO();
    await this.pool.query(
      `UPDATE mg_entity_fact SET emotional_weight = $1, emotional_valence = $2, updated_at = $3 WHERE uuid = $4`,
  ...
  async updateEngagementScore(factUuid: string, score: number): Promise<void> {
    ...
    const now = nowISO();
    await this.pool.query(
      `UPDATE mg_entity_fact SET engagement_score = $1, updated_at = $2 WHERE uuid = $3`,
      [score, now, factUuid]
    );
  }
  ...
  async countEntityFacts(entityUuid: string): Promise<number> {
    await this.ready();
    const res = await this.pool.query(
      `SELECT COUNT(*) as cnt FROM mg_entity_fact WHERE entity_id = $1`,
      [entityUuid]
    );
    return parseInt(res.rows[0]?.cnt ?? '0', 10);
  }
  ...
  async getEntityFirstFactDate(entityUuid: string): Promise<string | null> {
    await this.ready();
    const res = await this.pool.query(
      `SELECT MIN(created_at) as first_date FROM mg_entity_fact WHERE entity_id = $1`,
      [entityUuid]
    );
    return res.rows[0]?.first_date ?? null;
  }
