...
export interface Store {
  ...
  // Pin & Confirm (user-visible memory)
...
function buildFactFilterClauses(filter: FactFilter): { clauses: string; args: any[] } {
  ...
  if (filter.threadStatuses && filter.threadStatuses.length > 0) {
    clauses += ` AND thread_status IN (${filter.threadStatuses.map(() => '?').join(',')})`;
  ...
  if (filter.emotionalValences && filter.emotionalValences.length > 0) {
    clauses += ` AND emotional_valence IN (${filter.emotionalValences.map(() => '?').join(',')})`;
...
export class MemGStore implements Store {
    ...
    const tx = this.db.transaction(() => {
      for (const ddl of SCHEMA_DDL) {
        try {
          ...
          // Ignore "duplicate column" errors from ALTER TABLE migrations on existing DBs
  ...
  pinFact(factUuid: string): void {
    const now = nowISO();
    this.db
      .prepare(`UPDATE mg_entity_fact SET pinned = 1, significance = 10, expires_at = NULL, updated_at = ? WHERE uuid = ?`)
      .run(now, factUuid);
  }

  unpinFact(factUuid: string): void {
    const now = nowISO();
    this.db
      .prepare(`UPDATE mg_entity_fact SET pinned = 0, updated_at = ? WHERE uuid = ?`)
      .run(now, factUuid);
  }

  confirmFact(factUuid: string): void {
    const now = nowISO();
    this.db
      .prepare(`UPDATE mg_entity_fact SET confidence = 1.0, reinforced_at = ?, reinforced_count = reinforced_count + 1, updated_at = ? WHERE uuid = ?`)
      .run(now, now, factUuid);
  }

  // ---- Thread tracking ----

  updateThreadStatus(factUuid: string, status: string): void {
    const now = nowISO();
    this.db
      .prepare(`UPDATE mg_entity_fact SET thread_status = ?, updated_at = ? WHERE uuid = ?`)
      .run(status, now, factUuid);
  }

  listOpenThreads(entityUuid: string, limit: number): Fact[] {
    const effectiveLimit = limit > 0 ? limit : 20;
    const rows = this.db
      .prepare(
        `SELECT ${FACT_COLUMNS} FROM mg_entity_fact WHERE entity_id = ? AND thread_status = 'open' ORDER BY significance DESC, created_at DESC LIMIT ?`
      )
      .all(entityUuid, effectiveLimit) as any[];
    return rows.map(rowToFact);
  }
  ...
  updateEmotionalWeight(factUuid: string, weight: number, valence: string): void {
    const now = nowISO();
    this.db
      .prepare(`UPDATE mg_entity_fact SET emotional_weight = ?, emotional_valence = ?, updated_at = ? WHERE uuid = ?`)
      .run(weight, valence, now, factUuid);
  }

  updateEngagementScore(factUuid: string, score: number): void {
    const now = nowISO();
    this.db
      .prepare(`UPDATE mg_entity_fact SET engagement_score = ?, updated_at = ? WHERE uuid = ?`)
      .run(score, now, factUuid);
  }

  // ---- Entity stats ----

  countEntityFacts(entityUuid: string): number {
    const row = this.db
      .prepare(`SELECT COUNT(*) as cnt FROM mg_entity_fact WHERE entity_id = ?`)
      .get(entityUuid) as any;
    return row?.cnt ?? 0;
  }

  getEntityFirstFactDate(entityUuid: string): string | null {
    const row = this.db
      .prepare(`SELECT MIN(created_at) as first_date FROM mg_entity_fact WHERE entity_id = ?`)
      .get(entityUuid) as any;
