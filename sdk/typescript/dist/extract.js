 ...
 */
function isTrivialTurn(messages) {
 ...
 */
async function callLLM(apiKey, model, systemPrompt, userContent, provider) {
    let url;
    let headers;
    let body;
    if (provider === 'gemini') {
 ...
 * 8. Persist
 */
async function runExtraction(store, embedder, messages, entityUuid, apiKey, llmModel, llmProvider) {
    if (messages.length === 0)
