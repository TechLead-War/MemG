...
exports.dimensionMatch = dimensionMatch;
...
function termFrequencies(doc, terms) {
    const tf = new Map();
    for (const w of doc) {
...
function vectorScores(query, candidates) {
...
function bm25Scores(queryText, candidates) {
    ...
    const k1 = 1.2;
    const b = 0.75;
    const raw = new Array(docs.length).fill(0);
    let peak = 0;
...
function blendWeights(queryText) {
 ...
 */
class HybridEngine {
    rank(query, queryText, candidates, limit, threshold) {
