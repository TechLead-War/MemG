...
import { cosineSimilarity } from './search.mjs';
import { defaultContentKey } from './store.mjs';
 ...
 */
export function isTrivialTurn(messages) {
 ...
 */
function ttlForSignificance(sig) {
    if (sig >= 10)
 ...
 */
export async function callLLM(apiKey, model, systemPrompt, userContent, provider) {
    let url;
    let headers;
    let body;
    if (provider === 'gemini') {
 ...
 */
function findSemanticMatch(embedding, existingFacts) {
    let best = null;
    let bestScore = 0;
    ...
    if (bestScore >= SEMANTIC_DEDUP_THRESHOLD)
        return best;
    return null;
}
/**
 ...
 */
function findSlotConflicts(slot, existing) {
    if (!slot)
        return [];
    return existing.filter((f) => f.slot === slot &&
 ...
 * 8. Persist
 */
export async function runExtraction(store, embedder, messages, entityUuid, apiKey, llmModel, llmProvider) {
    if (messages.length === 0)
        return { inserted: 0, reinforced: 0 };
    if (isTrivialTurn(messages))
        return { inserted: 0, reinforced: 0 };
    ...
    const existingFacts = await store.listFactsForRecall(entityUuid, { statuses: ['current'], excludeExpired: true }, 0);
    let inserted = 0;
    let reinforced = 0;
 ...
 */
async function reinforceAndPromote(store, existing, newExpiresAt) {
    const newCount = existing.reinforcedCount + 1;
    const shouldPromote = newCount >= PROMOTION_THRESHOLD && existing.significance < 10;
