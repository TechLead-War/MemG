 ...
 */
import { dimensionMatch, } from './search.mjs';
/**
 * Recall facts for an entity using hybrid search.
 */
export async function recallFacts(store, engine, embedder, queryText, entityUuid, limit, threshold, maxCandidates, filter) {
    let vectors;
    try {
 ...
 */
export async function recallFactsWithVector(store, engine, queryVec, queryModel, queryText, entityUuid, limit, threshold, maxCandidates, filter) {
 ...
 */
export async function recallSummaries(store, engine, embedder, queryText, entityUuid, limit, threshold) {
    let vectors;
    try {
        vectors = await embedder.embed([queryText]);
    }
 ...
 */
export async function recallSummariesWithVector(store, engine, queryVec, queryText, entityUuid, limit, threshold) {
    if (queryVec.length === 0)
        return [];
