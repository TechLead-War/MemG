...
class MySQLStore(Store):
    ...
    def __init__(self, dsn: str = "", **connect_kwargs) -> None:
        ...
        self._conn = mysql.connector.connect(autocommit=True, **connect_kwargs)
        self._create_schema()
    ...
    def prune_expired_facts(self, entity_uuid: str, now: Optional[datetime] = None) -> int:
        if now is None:
            now = _now()
        cur = self._conn.cursor()
        cur.execute("DELETE FROM mg_entity_fact WHERE entity_id = %s AND expires_at IS NOT NULL AND expires_at < %s", (entity_uuid, _fmt(now)))
        count = cur.rowcount
        cur.close()
        return count
    ...
    def ensure_session(self, entity_uuid: str, process_uuid: str, timeout_seconds: int = 1800) -> Tuple[Session, bool]:
        now = _now()
        cur = self._conn.cursor()
        cur.execute("SELECT uuid, entity_id, process_id, created_at, expires_at FROM mg_session WHERE entity_id = %s AND process_id = %s AND expires_at > %s ORDER BY created_at DESC LIMIT 1", (entity_uuid, process_uuid, _fmt(now)))
        row = cur.fetchone()
        if row:
        ...
        cur.execute("INSERT INTO mg_session (uuid, entity_id, process_id, created_at, expires_at) VALUES (%s, %s, %s, %s, %s)", (uid, entity_uuid, process_uuid, _fmt(now), _fmt(expires)))
        cur.close()
    ...
    def list_conversation_summaries(self, entity_uuid: str, limit: int = 100) -> List[Conversation]:
        ...
        rows = cur.fetchall()
        cur.close()
        return [Conversation(uuid=r[0], session_id=r[1], entity_id=r[2] or "", summary=r[3] or "", summary_embedding=_decode(r[4]), created_at=_dt_val(r[5]), updated_at=_dt_val(r[6])) for r in rows]
